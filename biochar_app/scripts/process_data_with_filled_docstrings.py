...
import zipfile
import numpy as np
from biochar_app.scripts.get_weather_data import get_weather_data
...
def rename_logger_columns(df, logger_name):
    """
    Renames columns in a logger DataFrame to include logger identifiers.
    
    Args:
    df (pd.DataFrame): Description of df.
    logger_name (str): Description of logger_name.
    
    Returns:
    pd.DataFrame or dict or None: Description of what is returned by the function.
    """
    rename_dict = {}
...
def read_logger_data(name, year):
    """
    Reads logger data from a .dat file and preprocesses it.
    
    Args:
    name (str): Description of name.
    year (int): Description of year.
...
def merge_all_loggers(year):
    """
    Merges all logger DataFrames for a given year into a single DataFrame.
...
def replace_bad_values(df):
    """
    Replaces extreme placeholder values (e.g., 999999) in numeric columns with NaN.
...
def add_swc(df):
    """
    Calculates soil water content (SWC) as a weighted sum of VWC columns.
...
def calculate_ratios(df):
    """
    Calculates ratios between biochar and non-biochar strips for various variables.
...
def aggregate(df, year):
    """
    Aggregates the DataFrame into 15min, hourly, daily, and monthly resolutions.
...
def save_outputs(year, aggregated):
    """
    Saves aggregated DataFrames to CSV and compresses them into ZIP archives.
    
    Args:
    year (int): Description of year.
    aggregated (Any): Description of aggregated.
...
def process_logger_and_climate_data(year):
    """
    Processes logger and weather data, merges, cleans, aggregates, and saves outputs.
