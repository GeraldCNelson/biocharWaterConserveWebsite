...
### Fix: unpack_collectdata_response KeyError: slice(2, None, None)

When calling `bus.unpack_collectdata_response(...)`, ensure you pass the entire packet object returned by `wait_packet`, not an individual message dict. For example:
```python
pkt = bus.wait_packet(bus.transaction)
rows = bus.unpack_collectdata_response(pkt)
```
Passing a wrong variable (e.g., `msg`) can lead to `KeyError: slice(2, None, None)` because the expected structure isn't met.
